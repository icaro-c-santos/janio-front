...
const CustomerForm: React.FC<CustomerFormProps> = ({ onSuccess, onCancel }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (formData.email && formData.email.trim() !== '') {
                ...
                if (!emailRegex.test(formData.email)) validationErrors.email = 'Formato de email inválido';
            }
            if (!formData.name || formData.name.trim() === '') validationErrors.name = 'Nome é obrigatório';
            if (!formData.type) validationErrors.type = 'Tipo é obrigatório';
    ...
    const resetForm = () => {
        setFormData({ name: '', email: '', type: 'PF' });
        setError(null);
        setIsSuccess(false);
        setFieldErrors({});
    };
    ...
    return (
        <Paper sx={{ p: 3 }}>
            ...
            {isSuccess && (<Alert severity="success" sx={{ mb: 2 }}>Cliente cadastrado com sucesso!</Alert>)}
                ...
                <Grid container spacing={3}>
                    ...
                    <Grid item xs={12}>
                        <Box sx={{ display: 'flex', gap: 2, justifyContent: 'flex-end' }}>
                            <Button variant="outlined" onClick={onCancel || resetForm} disabled={loading}>Cancelar</Button>
